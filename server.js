...
app.get("/", (req, res) => {
  res.json({ status: "Stripe proxy is running", version: "7.0" });
});
...
// Fetch one page of charges with full invoice expansion
app.get("/charges", async (req, res) => {
  ...
  // Expand invoice fully including line items, price recurring interval, and billing_reason
  params.append("expand[]", "data.invoice");
  params.append("expand[]", "data.invoice.lines");
  params.append("expand[]", "data.invoice.lines.data.price");
...
app.listen(PORT, () => {
  console.log(`Stripe proxy v7.0 running on port ${PORT}`);
});
